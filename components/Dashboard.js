...
import { generateStrategicInsight } from '../services/geminiService.js';

const Dashboard = ({ donors, donations }) => {
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <${KPICard}
                    ...
                    icon=${html`<${DollarSign} className="text-tsp-600" size=${24} />`}
                    subtext="Since June 2023"
                />
                <${KPICard}
                    title="Sandwich Impact"
                    value=${sandwichImpact.toLocaleString()}
                    icon=${html`<${Sandwich} className="text-orange-500" size=${24} />`}
                    subtext="~760 per $1k"
                />
                <${KPICard}
                    title="Total Donors"
                    value=${totalDonors.toString()}
                    icon=${html`<${Users} className="text-blue-500" size=${24} />`}
                    subtext=${`${recurringDonorsCount} Recurring`}
                />
                <${KPICard}
                    title="Avg. Gift"
                    value=${`$${avgDonation.toFixed(0)}`}
                    icon=${html`<${TrendingUp} className="text-purple-500" size=${24} />`}
...
const KPICard = ({ title, value, icon, subtext }) => html`
