...
import { generateDonorCommunication } from '../services/geminiService.js';
import { ArrowLeft, Mail, Phone, MapPin, DollarSign, Edit, Sparkles, Copy, Check } from 'lucide-react';
import { SANDWICHES_PER_THOUSAND } from '../constants.js';

const DonorProfile = ({ donor, donations, onBack }) => {
                    ...
                    ` : html`
                        <div className="space-y-4 animate-fade-in">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                >
                                    ${copied ? html`<${Check} size=${16}/> Copied` : html`<${Copy} size=${16}/> Copy to Clipboard`}
                                </button>
                            </div>
                        </div>
                    `}
                        ...
                        ${donations.map((donation) => html`
                            <div key=${donation.id} className="flex items-start gap-4 p-4 rounded-lg bg-gray-50 border border-gray-100 hover:bg-gray-100 transition-colors">
                                ...
                                <div className="flex-1">
                                    ...
                                    ${donation.notes && html`<p className="text-xs text-gray-400 mt-1 italic">"${donation.notes}"</p>`}
                                </div>
                            </div>
                        `)}
                    </div>
